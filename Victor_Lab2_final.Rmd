...
```{r echo=FALSE}
confint(mod_RI)
stdCoef.merMod(mod_RI)# standardised beta coefficients 
```
...
we used the`R2beta`. our output shows that cortisol_serum and pain_cat has higher influence at Rsq 0.12
...
#compare model fit 
based on the anova both AIC, BIC and Loglik, random intercept is better than than random slop
```{r echo=TRUE}
...
#Based on our graphs, there is no much difference between our two models, hence we choose the random intercept. random slop had no significant influence.  
